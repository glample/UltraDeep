...
def test_Hidden_layer():
    ...
    for k in xrange(5):
        ...
        # hidden layer
        hidden_layer = layer.Hidden_layer(input_dim, output_dim, bias, activation, 'test')
        ...
        for i in xrange(20):
            ...
            if i % 4 == 0:
                ...
                input_value = np.random.rand(np.random.randint(100), input_dim).astype(floatX)
            elif i % 4 == 2:
                input = T.tensor3('input_test')
                input_value = np.random.rand(np.random.randint(50), np.random.randint(50), input_dim).astype(floatX)
            else:
                input = T.tensor4('input_test')
                input_value = np.random.rand(np.random.randint(20), np.random.randint(20), np.random.randint(20), input_dim).astype(floatX)

            output = hidden_layer.link(input)

            expected_value = np.dot(input_value, hidden_layer.weights.get_value())
            if bias: expected_value += hidden_layer.bias.get_value()
            if activation == T.tanh: expected_value = np.tanh(expected_value)
            else: expected_value = expit(expected_value)

            assert expected_value.shape == input_value.shape[:-1] + (output_dim,)
            np.testing.assert_array_almost_equal(output.eval({input:input_value}), expected_value)
...
def test_Embedding_layer():
    ...
    for k in xrange(1):
        ...
        # embedding layer
        embedding_layer = layer.Embedding_layer(input_dim, output_dim, 'test')
        ...
        for i in xrange(40):
            ...
            if i % 4 == 0:
                ...
                input_value = np.random.randint(low=0, high=input_dim, size=(np.random.randint(low=1, high=50),)).astype(np.int32)
            elif i % 4 == 1:
                input = T.imatrix('input_test')
                input_value = np.random.randint(low=0, high=input_dim, size=(np.random.randint(low=1, high=40),np.random.randint(low=1, high=40),)).astype(np.int32)
            elif i % 4 == 2:
                input = T.itensor3('input_test')
                input_value = np.random.randint(low=0, high=input_dim, size=(np.random.randint(low=1, high=30),np.random.randint(low=1, high=30),np.random.randint(low=1, high=30),)).astype(np.int32)
            else:
                input = T.itensor4('input_test')
                input_value = np.random.randint(low=0, high=input_dim, size=(np.random.randint(low=1, high=20),np.random.randint(low=1, high=20),np.random.randint(low=1, high=20),np.random.randint(low=1, high=20),)).astype(np.int32)
...
def test_RNN():
    ...
    for k in xrange(5):
        ...
        for i in xrange(10):
            ...
            input = T.matrix('input_test')
            input_value = np.random.rand(np.random.randint(low=1, high=30), input_dim).astype(floatX)
            output = rnn.link(input)

            h_t = rnn.h_0.get_value()
            for i in xrange(input_value.shape[0]):
                h_t = np.dot(input_value[i], rnn.w_x.get_value()) + np.dot(h_t, rnn.w_h.get_value()) + rnn.b_h.get_value()
                if activation == T.tanh: h_t = np.tanh(h_t)
                else: h_t = expit(h_t)

            assert h_t.shape == (hidden_dim,)
            np.testing.assert_array_almost_equal(output.eval({input:input_value}), h_t)
    ...
    for k in xrange(5):
        ...
        for i in xrange(10):
            ...
            input = T.tensor3('input_test')
            input_value = np.random.rand(np.random.randint(low=1, high=10), np.random.randint(low=1, high=30), input_dim).astype(floatX)
            ...
            for i in xrange(input_value_dimshuffled.shape[0]):
                h_t = np.dot(input_value_dimshuffled[i], rnn.w_x.get_value()) + np.dot(h_t, rnn.w_h.get_value()) + rnn.b_h.get_value()
                if activation == T.tanh: h_t = np.tanh(h_t)
                else: h_t = expit(h_t)
...
def test_kmax_pooling_layer_1():
    ...
    input = T.tensor4('input_test')
    k_max = T.iscalar('')
    kmax_pooling_layer_old = pooling.kmax_pooling_layer_1(k_max)
    ...
    for i in xrange(1000):
        ...
        # random parameters
        input_value = np.random.rand(np.random.randint(1, 20), np.random.randint(1, 20), np.random.randint(10, 20), np.random.randint(1, 100)).astype(floatX)
...
def test_kmax_pooling_layer_2():
    ...
    input = T.tensor4('input_test')
    k_max = T.iscalar('')
    kmax_pooling_layer = pooling.kmax_pooling_layer_2(k_max)
...
def test_Conv1D_layer():
    ...
    for k in xrange(5):
        ...
        # hidden layer
        conv_layer = convolution.Conv1D_layer(nb_filters, stack_size, filter_height, wide, emb_dim, 'conv1d_layer')
        ...
        for i in xrange(20):
            ...
            # tests for dimension 1, 2, 3 and 4
            input = T.tensor4('input_test')
            input_value = np.random.rand(1, stack_size, np.random.randint(1, 30), emb_dim).astype(floatX)
            ...
            for i in xrange(nb_filters):
                for j in xrange(stack_size):
                    for k in xrange(emb_dim):
                        ...
                        expected_value[0][i][:,k] += convolve(input_value[0,j,:,k], filters[k,i,j].flatten(), mode = "full" if wide else "valid")
            ...
            np.testing.assert_array_almost_equal(output.eval({input:input_value}), expected_value, decimal=4)
...
def test_Conv2D_layer():
    ...
    for k in xrange(5):
        ...
        # hidden layer
        conv_layer = convolution.Conv2D_layer(nb_filters, stack_size, filter_height, filter_width, wide, "conv_layer")
        ...
        for i in xrange(20):
            ...
            # tests for dimension 1, 2, 3 and 4
            input = T.tensor4('input_test')
            input_value = np.random.rand(stack_size, np.random.randint(1, 20), np.random.randint(1, 20)).astype(floatX)
            ...
            np.testing.assert_array_almost_equal(output.eval({input:input_value_4d})[0][0], expected_value, decimal=6)
...
def test_Conv1D_layer_kmax_pooling():
    ...
    for k in xrange(5):
        ...
        # hidden layer
        conv_layer = convolution.Conv1D_layer_kmax_pooling(nb_filters, stack_size, filter_height, wide, emb_dim, 'conv1d_layer')
        ...
        for i in xrange(20):
            ...
            input_value = np.random.rand(1, stack_size, np.random.randint(20, 30), emb_dim).astype(floatX)
            while (not wide) and input_value.shape[2] < filters.shape[3]:
                input_value = np.random.rand(1, stack_size, np.random.randint(20, 30), emb_dim).astype(floatX)
            ...
            np.testing.assert_array_almost_equal(conv_layer.output.eval({input:input_value}), expected_value, decimal=4)
...
def test_Conv2D_layer_kmax_pooling():
    ...
    for k in xrange(5):
        ...
        # hidden layer
        conv_layer = convolution.Conv2D_layer_kmax_pooling(nb_filters, stack_size, filter_height, filter_width, wide, "conv_layer_kmax_pooling")
        ...
        for i in xrange(20):
            ...
            np.testing.assert_array_almost_equal(output.eval({input:input_value_4d}), expected_value, decimal=4)
